...
export default function InteractiveEventConfigurator() {
  ...
  const handleSelectExperience = (type: BaseExperienceType) => {
    ...
    if (type === "solo") {
      ...
      // Reset Scale so viewer MUST click Step 2 manually
      setEventScale(null);
      setMascotsCount(1);
      setVideographers(0);
      setPhotographers(0);
...
Please confirm date lock & deposit instructions!`;
  ...
  // Gate check to render lower steps
  const canShowLowerSteps = baseExperience === "solo" || (baseExperience === "vip" && eventScale !== null);
    ...
    >
      ...
      <div className="max-w-4xl mx-auto w-full space-y-10 text-center">
        ...
        <div className="space-y-8 text-left">
          ...
          {/* UNSELECTED SCALE CALLOUT BOX (Shows when VIP is selected but Event Scale is null) */}
          {baseExperience === "vip" && !eventScale && (
            <div className="p-6 text-center border-2 border-dashed border-amber-500/40 rounded-2xl bg-amber-500/5 text-amber-300/90 my-6 animate-pulse flex flex-col items-center gap-2">
              <ArrowUpCircle className="w-7 h-7 text-amber-400" />
              <p className="font-bold text-sm sm:text-base text-amber-200">
                Please select your Event Scale & Capacity in Step 2
              </p>
              <p className="text-xs text-slate-400">
                Select your guest count above to automatically calculate your required media crew and pricing breakdown.
              </p>
            </div>
          )}

          {/* STEP 3, STEP 4 & COST BREAKDOWN (Only rendered when scale is selected for VIP or Solo is picked) */}
          {canShowLowerSteps && (
            <>
              {/* STEP 3: CREW & DELIVERABLES */}
              <div className="bg-[#0B101D] border border-slate-800/90 rounded-2xl p-5 sm:p-8 shadow-2xl space-y-6">
                <div className="flex items-center justify-center gap-3">
                  <span className="flex items-center justify-center w-8 h-8 rounded-full bg-emerald-500/20 text-emerald-400 font-bold text-sm border border-emerald-500/40">
                    3
                  </span>
              ...
              {/* STEP 4: CITY & DATE */}
              <div className="bg-[#0B101D] border border-slate-800/90 rounded-2xl p-5 sm:p-8 shadow-2xl space-y-5">
                <div className="flex items-center justify-center gap-3">
                  <span className="flex items-center justify-center w-8 h-8 rounded-full bg-emerald-500/20 text-emerald-400 font-bold text-sm border border-emerald-500/40">
                    4
                  </span>
